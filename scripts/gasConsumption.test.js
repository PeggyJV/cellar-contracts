...
async function main() {
  ...
  tx = await cellar["deposit(uint256)"](
    ethers.BigNumber.from(10).pow(6).mul(500)
  );
  gasUsedLog("cellar.deposit", tx);

  tx = await cellar.swap(
    usdc.address,
    dai.address,
  ...
  tx = await cellar["withdraw(uint256)"](
    ethers.BigNumber.from(10).pow(6).mul(700)
  );
  ...
  tx = await cellar.redeemFromAave(
    usdc.address,
  ...
  await cellar.redeemFromAave(
    dai.address,
    ethers.BigNumber.from(10).pow(18).mul(50)
  );
}
