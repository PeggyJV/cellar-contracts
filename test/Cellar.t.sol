...
import { Registry, PriceRouter, IGravity } from "src/Registry.sol";
import { SwapRouter, IUniswapV2Router, IUniswapV3Router } from "src/modules/SwapRouter.sol";
...
contract CellarTest is Test {
    ...
    function setUp() external {
        ...
        exchange.setExchangeRate(address(USDC), address(USDC), 1e6);
        exchange.setExchangeRate(address(WETH), address(WETH), 1e18);
        exchange.setExchangeRate(address(WBTC), address(WBTC), 1e8);

        exchange.setExchangeRate(address(USDC), address(WETH), 0.0005e18);
        exchange.setExchangeRate(address(WETH), address(USDC), 2000e6);

        exchange.setExchangeRate(address(USDC), address(WBTC), 0.000033e8);
        exchange.setExchangeRate(address(WBTC), address(USDC), 30_000e6);

        exchange.setExchangeRate(address(WETH), address(WBTC), 0.06666666e8);
        exchange.setExchangeRate(address(WBTC), address(WETH), 15e18);

        // Setup Registry and modules:
        swapRouter = new SwapRouter(uniswapV2Router, uniswapV3Router);
        priceRouter = PriceRouter(address(new MockPriceRouter(exchange)));
        ...
        for (uint256 i; i < positions.length; i++) {
            MockERC20 asset = MockERC20(address(ERC4626(positions[i]).asset()));
            asset.mint(address(exchange), type(uint112).max);
        }
    }
    ...
    //     uint256 expectedAssetsOut = exchange.quote(10_000e6, path);
    ...
    //     expectedAssetsOut = exchange.quote(assetsRebalanced, path);
    ...
    //     uint256 expectedAssetsOut = exchange.quote(assets, path);
    ...
    //         uint256 assets = exchange.convert(address(USDC), address(positionAsset), 100e6);
    ...
    //         uint256 assets = exchange.convert(address(USDC), address(positionAsset), 50e6);
    ...
    //         wethCLR,
    //         exchange.convert(address(USDC), address(WETH), 100e6),
    //         address(this),
    //         address(this)
    //     );
    ...
    //     uint256 assets = exchange.convert(address(USDC), address(WETH), 100e6);
    ...
    //     // Deposit assets into position before distrusting.
    //     uint256 assets = exchange.convert(address(USDC), address(WBTC), 100e6);
