...
contract CurveAdaptorTest is MainnetStarterTest, AdaptorHelperFunctions {
    ...
    function setUp() external {
        ...
        slippageCoins.push(ERC20(address(0)));
        slippageCoins.push(ERC20(address(0)));
    }

    // ========================================= HAPPY PATH TESTS =========================================

    // EIN the problem children pools according to Crispy: OETH, mkUSD, ETHx, yETH --> EMAs can be used for them, but mkUSD is messy because it is based on a different curve market pool.
    // EIN - problem children according to me to be aware of: sFrax, sDAI, and stETH bc they use different compiler versions, and sDAI uses CurveStableSwap, not just StableSwap (maybe there is no difference).

    // see helper used for full description of what is being tested here.
    // EIN QUESTIONS - I guess we are not caring about vCRV boosts working properly or not. We trust that it is and that the reward claiming that is carried out with this adaptor will capture any boosted rewards too if we implement abilities for the cellar to do vCRV voting and boosting.
    // EIN QUESTIONS - what happens if zero is passed in as a param?
    // EIN QUESTIONS -  what about values larger than 1million?
    // EIN QUESTIONS -  are there ways that pricing can be manipulated in a "stateful" way to attack through the curve adaptor? What happens if/when curve pool liquidity dries up? In general we need mitigation methods.
    // EIN QUESTIONS - why these varying amounts of tolerances used as params btw different happy path tests with different pools?
    ...
    function testWithdrawLogic(uint256 assets) external {
        ...
        // EIN QUESTION - So is the code above this line really necessary? We should know that the cellar can actually successfully deposit and redeem with a liquid position such as usdcCrvUsdPool
        ...
        // If a cellar tried to withdraw from the Curve Position it would revert. EIN QUESTION - WHY? Is it not set as liquid, so why would this one revert? May you elaborate and also specify the revert statement here and on the other test?
    ...
    function testWithdrawWithReentrancy1(uint256 assets) external {
        assets = bound(assets, 1e6, 1_000_000e6);
        _checkForReentrancyOnWithdraw(assets, EthFrxethPool, EthFrxethToken);
    }

    function testWithdrawWithReentrancy2(uint256 assets) external {
        assets = bound(assets, 1e6, 1_000_000e6);
        _checkForReentrancyOnWithdraw(assets, EthStethNgPool, EthStethNgToken);
    }

    function testWithdrawWithReentrancy3(uint256 assets) external {
        assets = bound(assets, 1e6, 1_000_000e6);
        _checkForReentrancyOnWithdraw(assets, EthOethPool, EthOethToken);
    }

    function testWithdrawWithReentrancy4(uint256 assets) external {
        assets = bound(assets, 1e6, 100_000e6);
        _checkForReentrancyOnWithdraw(assets, EthEthxPool, EthEthxToken);
    }
    ...
    function testSlippageRevertsNoETH(uint256 assets) external {
        ...
        slippageCoins[0] = coins0;
        slippageCoins[1] = coins1;
    ...
    function add_liquidity(uint256[2] memory amounts, uint256) external payable {
        ...
        if (address(slippageCoins[0]) != curveAdaptor.CURVE_ETH()) {
            uint256 coins0Balance = slippageCoins[0].balanceOf(msg.sender);
            deal(address(slippageCoins[0]), msg.sender, coins0Balance - amounts[0]);
        } else slippageCoins[0] = WETH;
        if (address(slippageCoins[1]) != curveAdaptor.CURVE_ETH()) {
            uint256 coins1Balance = slippageCoins[1].balanceOf(msg.sender);
            deal(address(slippageCoins[1]), msg.sender, coins1Balance - amounts[1]);
        } else slippageCoins[1] = WETH;

        // Get value out.
        uint256[] memory coinAmounts = new uint256[](2);
        coinAmounts[0] = amounts[0];
        coinAmounts[1] = amounts[1];
        uint256 valueOut = priceRouter.getValues(slippageCoins, coinAmounts, ERC20(slippageToken));
    ...
    function remove_liquidity(uint256 lpAmount, uint256[2] memory) external {
        ...
        else valueOut = priceRouter.getValue(ERC20(slippageToken), lpAmount, slippageCoins[0]);
        ...
        if (address(slippageCoins[0]) != curveAdaptor.CURVE_ETH()) {
            uint256 coins0Balance = slippageCoins[0].balanceOf(msg.sender);
            deal(address(slippageCoins[0]), msg.sender, coins0Balance + valueOut);
        } else {
    ...
    /**
     * Deploys a cellar w/ Curve LPT as Accounting Asset. Adds curveAdaptor to cellar catalogue. Deposits `assets` amount into cellar from address(this). Upon depositing it into the cellar, that has the holding position of a CurvePoolAdaptor position where the resultant curve LPT will be deposited into the gauge if there is a gauge.
     * It checks that the `asset` amount within the gauge has been deposited, with initial Assets.
     * THEN it makes an adaptorCall to pull half of assets from gauge.
     * Cellar now has half staked, half unstaked Curve LPT.
     * test address then redeems all shares. AssertChecks that all `assets` has been withdrawn from Cellar.
     * Big takeaways: LPTs did not increase from OG `assets` amount. Mutative strategist functions worked and CurveLPTs were always accounted for (whether staked or not).
     */
    ...
    /**
     * **tldr - CHECKS: addLiquidity() w/ one token, addLiquidity() w/ both tokens, staking LPT, unstaking LPT, claiming CRV rewards, withdrawLiquidity()**
     * Deals out `assets` amount of USDC to address(this), it deposits into `assets` into cellar. Deals, or steals, whatever ERC20 coins0 is from curve pool to cellar - the amount is the coins0 equivalent to `assets` in USDC, converted.
     * AdaptorCall with CurveAdaptor to addLiquidity() w/ orderedTokenAmounts where [0] is half and  [1] is 0 in `assets`
     * Checks for CurveLPBalance. Should have assets / 2 amount of Curve LPT.
     * Then adds `assets/4` for both coins[0] and coins[1] to the curve pool via addLiquidity w/ curve adaptor.
     * Assert checks that tolerance was not surpassed
     * Strategist stakes LP using `stakeInGauge()` strategist function from curve adaptor. Check that it worked.
     * Unstake half of `assets` using `unstakeFromGauge()` and check.
     * Zeroes out LPTs & CRV because it will test claiming rewards and unstaking all LPTs.
     * Removes a specified `amountToPull` from Curve
     * NOTE: it looks like we remove liquidity in equal balance (all constituent tokens), or as ETH, NOT as oneToken, etc.
     * Checks that it got the right amount of constituent tokens out of Curve finally.
     *
     */
    function _manageLiquidityIn2PoolNoETH(
        ...
        assertGt(ERC20(token).balanceOf(address(cellar)), 0, "Should have added liquidity"); // TODO: this check would pass from before... need to make this assertGt(ERC20(token).balanceOf(address(cellar)), cellarCurveLPBalance);
        ...
        deal(address(CRV), address(cellar), 0); // TODO: EIN this doesn't zero out LPT balance, it zeroes out CRV token balance.
        ...
        // TODO: EIN - assert(ERC20(token).balanceOf(address(cellar), assets, "Cellar should have all LPTs that it is owed from curve market and gauge."));
    ...
    /**
     * Basically does the same thing as the above helper except uses the appropriate curve adaptor functions for handling native ETH. So this is more about handling native ETH and if the method works which I need more clarification from Crispy on.
     */
    function _manageLiquidityIn2PoolWithETH(
        ...
        if (address(coins[0]) == curveAdaptor.CURVE_ETH()) coins[0] = WETH; // EIN QUESTION: Ah, alright, so Curve creates ETH pools w/ address CURVE_ETH as the address. Then we treat it as WETH.
        ...
        // Strategist rebalances into LP , single asset. EIN QUESTION - so here, we are using the native wrapper (WETH as per this test file, but determined by the constructor for the curve Adaptor in question). The CurveAdaptor adds ETH via proxy. Need to discuss this with Crispy more. In general, the point of this is ?? I think it is simply to handle native ETH by sending WETH instead.
        {
            Cellar.AdaptorCall[] memory data = new Cellar.AdaptorCall[](1);
     ...
     * 
     */
    function _checkForReentrancyOnWithdraw(uint256 assets, address pool, address token) internal {
        deal(address(USDC), address(this), assets);
        cellar.deposit(assets, address(this));
        ...
        // EIN QUESTION - please elaborate on all of this.
        attackCellar = false;
        curvePool.remove_liquidity(1e18, [uint256(0), 0]);
