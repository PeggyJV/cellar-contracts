...
import { EtherFiStakingAdaptor, StakingAdaptor, IWithdrawRequestNft } from "src/modules/adaptors/Staking/EtherFiStakingAdaptor.sol";
...
contract EtherFiStakingAdaptorTest is MainnetStarterTest, AdaptorHelperFunctions {
    ...
    uint32 public weethPosition = 2;
    uint32 public etherFiPosition = 3;
    ...
    function testMintAndWrap(uint256 mintAmount) external {
        ...
        _mintDeriviativeAndWrap(mintAmount);
        assertApproxEqAbs(
        ...
        uint256 expectedWrappedDerivativeAmount = priceRouter.getValue(primitive, mintAmount, wrappedDerivative);
        assertApproxEqRel(
            wrappedDerivative.balanceOf(address(cellar)),
            expectedWrappedDerivativeAmount,
            0.01e18,
    ...
    function testBurn(uint256 mintAmount) external {
        ...
        _unwrapAndStartDeriviativeBurnRequest(type(uint256).max);
    ...
    function _unwrapAndStartDeriviativeBurnRequest(uint256 burnAmount) internal {
        Cellar.AdaptorCall[] memory data = new Cellar.AdaptorCall[](1);
        bytes[] memory adaptorCalls = new bytes[](2);
        adaptorCalls[0] = _createBytesDataToUnwrap(burnAmount);
        adaptorCalls[1] = _createBytesDataToRequestBurn(burnAmount);
    ...
    function _mintDeriviativeAndWrap(uint256 mintAmount) internal {
        // Rebalance Cellar to mint derivative.
        Cellar.AdaptorCall[] memory data = new Cellar.AdaptorCall[](1);
        bytes[] memory adaptorCalls = new bytes[](2);
        adaptorCalls[0] = _createBytesDataToMint(mintAmount);
        adaptorCalls[1] = _createBytesDataToWrap(type(uint256).max);
