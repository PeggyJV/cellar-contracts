...
contract WstEthOracleTest is Test {
  ...
  function testAddWstEthAssetAndComparePriceToAaveOracle() external {
    PriceRouter.AssetSettings memory settings = PriceRouter.AssetSettings(CHAINLINK_DERIVATIVE, address(wstEthOracle));
    PriceRouter.ChainlinkDerivativeStorage memory stor = PriceRouter.ChainlinkDerivativeStorage(90e18, 0.1e18, 0, true);
    ...
    uint256 expectedPriceInUsdc = expectedWstEthPriceInBase.mulDivDown(1e6, aaveOracle.getAssetPrice(address(USDC)));

    uint256 expectedPriceInWeth = expectedWstEthPriceInBase.mulDivDown(1e18, aaveOracle.getAssetPrice(address(WETH)));
