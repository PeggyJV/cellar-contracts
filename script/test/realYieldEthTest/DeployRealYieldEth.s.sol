...
contract DeployRealYieldEthScript is Script, TEnv {
    ...
    function run() external {
        ...
        uint32 aWethPosition = registry.trustPosition(address(aaveV3ATokenAdaptor), abi.encode(address(aWETH)), 0, 0);
        uint32 aRethPosition = registry.trustPosition(address(aaveV3ATokenAdaptor), abi.encode(address(aRETH)), 0, 0);
        uint32 aCBethPosition = registry.trustPosition(address(aaveV3ATokenAdaptor), abi.encode(address(aCBETH)), 0, 0);
        ...
        uint32 debtRethPosition = registry.trustPosition(
            address(aaveV3DebtTokenAdaptor),
            abi.encode(address(dRETH)),
            0,
            0
        );
        uint32 debtCBethPosition = registry.trustPosition(
            address(aaveV3DebtTokenAdaptor),
            abi.encode(address(dCBETH)),
            0,
            0
        );
