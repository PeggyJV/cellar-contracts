 ...
 */
contract AaveV2StablecoinCellarGasTest is IAaveV2StablecoinCellar, ERC20, ReentrancyGuard, Ownable {
     ...
     * @param amountOutMinimum minimum amount of tokens returned
     * @param useUniswap whether to use Uniswap or Sushiswap
     * @param useMultihopSwap useMultihopSwap
     * @return amountOut amount of tokens received after swap
     */
    function swapForGasTest(
        address[] memory path,
        uint256 amountIn,
        uint256 amountOutMinimum,
        bool useUniswap,
        bool useMultihopSwap
        ...
        if (path.length > 2 || useMultihopSwap){
            if (useUniswap) {
                bytes memory encodePackedPath = abi.encodePacked(tokenIn);
                for (uint256 i = 1; i < path.length; i++) {
                    encodePackedPath = abi.encodePacked(
            ...
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
                .ExactInputSingleParams({
                    ...
                    amountIn: amountIn,
                    amountOutMinimum: amountOutMinimum,
                    sqrtPriceLimitX96: 0  // Can be used to determine limits on the pool prices which cannot  be exceeded by the swap. If you set it to 0, it's ignored.
                });
