...
describe("AaveStablecoinCellar", () => {
    ...
    beforeEach(async () => {
        ...
        incentivesController = await ethers.getContractAt("IAaveIncentivesController", incentivesControllerAddress);

        dataProvider = await ethers.getContractAt("IAaveProtocolDataProvider", dataProviderAddress);
        ...
        await usdc.connect(alice).approve(cellar.address, ethers.BigNumber.from(10).pow(6).mul(10000));
        await dai.connect(alice).approve(cellar.address, ethers.BigNumber.from(10).pow(18).mul(10000));
        await weth.connect(alice).approve(cellar.address, ethers.BigNumber.from(10).pow(18).mul(10000));
        await usdt.connect(alice).approve(cellar.address, ethers.BigNumber.from(10).pow(6).mul(10000));
    ...
    describe("deposit", () => {
        it("should mint correct amount of shares to user", async () => {
            ...
            expect(await cellar.balanceOf(owner.address)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));

            // add $50 of inactive assets in cellar
            await cellar.connect(alice)["deposit(uint256)"](ethers.BigNumber.from(10).pow(6).mul(50));
            // expect 50 shares = 100 total shares * ($50 / $100) to be minted
            expect(await cellar.balanceOf(alice.address)).to.eq(ethers.BigNumber.from(10).pow(6).mul(50));
        });
        ...
        it("should transfer input token from user to cellar", async () => {
            ...
            expect(initialUserBalance.sub(updatedUserBalance)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));
            expect(updatedCellarBalance.sub(initialCellarBalance)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));
        });
        ...
        it("should swap input token for current lending token if not already", async () => {
            ...
            expect(initialUserBalance.sub(updatedUserBalance)).to.eq(ethers.BigNumber.from(10).pow(18).mul(100));
            // expect at least $95 to have been received by cellar
            expect(updatedCellarBalance.sub(initialCellarBalance)).to.be.at.least(ethers.BigNumber.from(10).pow(6).mul(95));
            ...
            expect(await cellar.balanceOf(owner.address)).to.be.at.least(ethers.BigNumber.from(10).pow(6).mul(95));
        });

        it("should mint shares to receiver instead of caller if specified", async () => {
             // owner mints to alice
            await cellar["deposit(uint256,address)"](ethers.BigNumber.from(10).pow(6).mul(100), alice.address);
            // expect alice receives 100 shares
            expect(await cellar.balanceOf(alice.address)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));
        ...
        it("should deposit all user's balance if tries to deposit more than they have", async () => {
            ...
            await cellar["deposit(uint256)"](initialUserBalance.add(ethers.BigNumber.from(10).pow(6).mul(1000)));
        ...
        it("should emit Deposit event", async () => {
            await expect(cellar["deposit(uint256,address)"](ethers.BigNumber.from(10).pow(6).mul(100), alice.address))
                .to.emit(cellar, "Deposit")
                .withArgs(owner.address, alice.address, ethers.BigNumber.from(10).pow(6).mul(100), ethers.BigNumber.from(10).pow(6).mul(100));
        });
    });

    describe("depositAndEnter", () => {
        it("should deposit directly into Aave strategy", async () => {
            await cellar.depositAndEnter(usdc.address, ethers.BigNumber.from(10).pow(6).mul(1000), ethers.BigNumber.from(10).pow(6).mul(1000), owner.address);
            expect(await aUSDC.balanceOf(cellar.address)).to.eq(ethers.BigNumber.from(10).pow(6).mul(1000));
        });
    });
    ...
    describe("withdraw", () => {
        beforeEach(async () => {
            ...
            await cellar["deposit(uint256)"](ethers.BigNumber.from(10).pow(6).mul(100));
            await cellar.connect(alice)["deposit(uint256)"](ethers.BigNumber.from(10).pow(6).mul(100));
        });
        ...
        it("should withdraw correctly when called with all inactive shares", async () => {
            ...
            await cellar["withdraw(uint256)"](ethers.BigNumber.from(10).pow(6).mul(1000));
            const ownerUpdatedBalance = await usdc.balanceOf(owner.address);
            // expect owner receives desired amount of tokens
            expect(ownerUpdatedBalance.sub(ownerInitialBalance)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));
            ...
            await cellar.connect(alice)["withdraw(uint256)"](ethers.BigNumber.from(10).pow(6).mul(100));
            const aliceUpdatedBalance = await usdc.balanceOf(alice.address);
            // expect alice receives desired amount of tokens
            expect(aliceUpdatedBalance.sub(aliceInitialBalance)).to.eq(ethers.BigNumber.from(10).pow(6).mul(100));
        ...
        it("should withdraw correctly when called with all active shares", async () => {
            ...
            await cellar["withdraw(uint256)"](ethers.BigNumber.from(10).pow(6).mul(125));
            const ownerUpdatedBalance = await usdc.balanceOf(owner.address);
            // expect owner receives desired amount of tokens
            expect(ownerUpdatedBalance.sub(ownerInitialBalance)).to.eq((ethers.BigNumber.from(10).pow(6).mul(125)));
            // expect all owner's shares to be burned
            expect(await cellar.balanceOf(owner.address)).to.eq(0);

            const aliceInitialBalance = await usdc.balanceOf(alice.address);
            // alice should be able redeem all shares for $125 (50% of total)
            await cellar.connect(alice)["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(125)));
            const aliceUpdatedBalance = await usdc.balanceOf(alice.address);
            // expect alice receives desired amount of tokens
            expect(aliceUpdatedBalance.sub(aliceInitialBalance)).to.eq((ethers.BigNumber.from(10).pow(6).mul(125)));
            // expect all alice's shares to be burned
            expect(await cellar.balanceOf(alice.address)).to.eq(0);
        });
        ...
        it("should withdraw correctly when called with active and inactive shares", async () => {
            ...
            // mimic growth from $200 -> $250 (1.25x increase) while in strategy
            await lendingPool.setLiquidityIndex(BigNumber.from("1250000000000000000000000000"));

            // owner adds $100 of inactive assets
            await cellar["deposit(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(100)));
            // alice adds $75 of inactive assets
            await cellar.connect(alice)["deposit(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(75)));

            const ownerInitialBalance = await usdc.balanceOf(owner.address);
            // owner should be able redeem all shares for $225 ($125 active + $100 inactive)
            await cellar["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(225)));
            const ownerUpdatedBalance = await usdc.balanceOf(owner.address);
            // expect owner receives desired amount of tokens
            expect(ownerUpdatedBalance.sub(ownerInitialBalance)).to.eq((ethers.BigNumber.from(10).pow(6).mul(225)));
            // expect all owner's shares to be burned
            expect(await cellar.balanceOf(owner.address)).to.eq(0);

            const aliceInitialBalance = await usdc.balanceOf(alice.address);
            // alice should be able redeem all shares for $200 ($125 active + $75 inactive)
            await cellar.connect(alice)["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(200)));
            const aliceUpdatedBalance = await usdc.balanceOf(alice.address);
            // expect alice receives desired amount of tokens
            expect(aliceUpdatedBalance.sub(aliceInitialBalance)).to.eq((ethers.BigNumber.from(10).pow(6).mul(200)));
            // expect all alice's shares to be burned
            expect(await cellar.balanceOf(alice.address)).to.eq(0);
        });
        ...
        it("should use and store index of first non-zero deposit", async () => {
            ...
            await cellar["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(100)));
            ...
            await cellar.connect(alice)["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(50)));
        ...
        it("should withraw all user's assets if tries to withdraw more than they have", async () => {
            ...
            await cellar.connect(alice)["withdraw(uint256)"]((ethers.BigNumber.from(10).pow(6).mul(150)));
        ...
        it("should emit Withdraw event", async () => {
            await expect(
                ...
                .withArgs(owner.address, alice.address, owner.address, (ethers.BigNumber.from(10).pow(6).mul(100)), (ethers.BigNumber.from(10).pow(6).mul(100)));
        });
    });
    ...
    describe("enterStrategy", () => {
      ...
      it("should return correct amount of aTokens to cellar", async () => {
        expect(await aUSDC.balanceOf(cellar.address)).to.eq(ethers.BigNumber.from(10).pow(6).mul(200));
      });
