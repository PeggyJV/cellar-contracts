...
def test_empty(USDC, WETH, accounts, SwapRouter, CellarPoolShareContract):
    SwapRouter.exactOutputSingle([WETH, USDC, 3000, accounts[2], 2 ** 256 - 1, 100000 * 10 ** 6, 50 * 10 ** 18, 0], {"from": accounts[2], "value": 50 * 10 ** 18})
    USDC.approve(CellarPoolShareContract, 100000 * 10 ** 6, {"from": accounts[2]})
    USDC_amount = 100000 * 10 ** 6
    ETH_amount = 50 * 10 ** 18
    cellarAddParams = [USDC_amount, ETH_amount, 0, 0, accounts[2], 2 ** 256 - 1]
    CellarPoolShareContract.addLiquidityEthForUniV3(cellarAddParams, {"from": accounts[2], "value": 50 * 10 ** 18})
    CellarPoolShareContract.setValidator(accounts[2], True, {"from": accounts[0]})
    for i in range(3):
        SwapRouter.exactOutputSingle([WETH, USDC, 3000, accounts[3], 2 ** 256 - 1, 100000 * 10 ** 6, 50 * 10 ** 18, 0], {"from": accounts[3], "value": 50 * 10 ** 18})
        USDC.approve(SwapRouter, 100000 * 10 ** 6, {"from": accounts[3]})
        SwapRouter.exactInputSingle([USDC, WETH, 3000, accounts[3], 2 ** 256 - 1, 100000 * 10 ** 6, 5 * 10 ** 18, 0], {"from": accounts[3]})
