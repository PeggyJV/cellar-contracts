...
contract SinglePositionUniswapV3PositionTracker {
    ...
    error SinglePositionUniswapV3PositionTracker__CallerDoesNotOwnTokenId();
    error SinglePositionUniswapV3PositionTracker__CallerOwnsTokenId();
    error SinglePositionUniswapV3PositionTracker__IndexAlreadyUsed();
    error SinglePositionUniswapV3PositionTracker__TokenIdMustBeOwnedByDeadAddress();
    error SinglePositionUniswapV3PositionTracker__TokenIdNotFound();
    ...
    function addPositionToTracker(uint256 tokenId, address pool, uint256 index) external {
        if (positionManager.ownerOf(tokenId) != msg.sender)
            revert SinglePositionUniswapV3PositionTracker__CallerDoesNotOwnTokenId();

        if (callerToPoolToIndexToPositionId[msg.sender][pool][index] != 0)
            revert SinglePositionUniswapV3PositionTracker__IndexAlreadyUsed();
    ...
    function removePositionFromTracker(address pool, uint256 index) external returns (uint256 tokenId) {
        ...
        if (tokenId == 0) revert SinglePositionUniswapV3PositionTracker__TokenIdNotFound();
    ...
    function removePositionThatIsNotOwnedByCaller(address pool, uint256 index) external {
        ...
        if (positionManager.ownerOf(tokenId) == msg.sender)
            revert SinglePositionUniswapV3PositionTracker__CallerOwnsTokenId();
